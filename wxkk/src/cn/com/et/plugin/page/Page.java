...
public class Page {
	...
	private String pageCol2Str;		//最终页面显示的底部翻页导航，详细见：getPageCol2Str();
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			if(currentPage==1){
				...
				sb.append("	<li><input type=\"text\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;float:left\" placeholder=\"页码\" maxlength=\"9\" onkeyup=\"this.value=this.value.replace(/\\D/g,'')\" onafterpaste=\"this.value=this.value.replace(/\\D/g,'')\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\" style=\"background-position: 0px 0px;\" class=\"btn   btn-danger\">跳转</a></li>\n");
	...
	public String getPageCol2Str() {
		...
		if(totalResult>0){
			...
			for(int i=startTag; i<=totalPage && i<=endTag; i++){
				if(currentPage==i)
					sb.append("<li><a><font color='#ee3d3c'>"+i+"</font></a></li>\n");
				else
					sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"nextPage("+i+")\">"+i+"</a></li>\n");
			}
			*/
		...
		pageCol2Str = sb.toString();
		return pageCol2Str;
	}
	public void setPageCol2Str(String pageCol2Str) {
		this.pageCol2Str = pageCol2Str;
	}
